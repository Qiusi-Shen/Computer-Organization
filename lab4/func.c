...
#pragma omp parallel for num_threads(30) private(j, index_X, index_Y)
   	for(i = 0; i<n; i++)
	  {
   		for(j = 0; j < Ones; j++)
		  {
...
void func2(double *weights, double *probability, int n)
{
	...
	#pragma omp parallel for num_threads(30) default(shared) private (i) reduction(+:sumWeights)
   	for(i = 0; i < n; i++)
...
void func3(double *arrayX, double *arrayY, double *weights, double *x_e, double *y_e, int n)
{
    ...
    int i;
 #pragma omp parallel for num_threads(30) default(shared) private (i) reduction(+:estimate_x,estimate_y)
	for(i = 0; i < n; i++){
...
{
	int i, j;
#pragma omp parallel for num_threads(30) default(shared) private (j,i)
	for(j = 0; j < n; j++){
